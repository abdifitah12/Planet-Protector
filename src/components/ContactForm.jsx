...
export default function ContactForm() {
  ...
  async function submit(e) {
    ...
    if (!f.name || !f.email || !f.phoneNumber || !f.address) {
      setMsg({ t:"error", x:"Name, email, phone and address are required." }); return;
    }
    try {
      setBusy(true);
  ...
  return (
    <section id="contact" className="py-16 bg-emerald-50">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        {msg && (
          <div className={`mt-4 rounded-xl p-3 text-sm ${msg.t==="ok"?"bg-emerald-100 text-emerald-900":"bg-red-100 text-red-800"}`}>
            {msg.x}
          </div>
        )}
        ...
        <form onSubmit={submit} className="mt-6 grid gap-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="name" value={f.name} onChange={on} className="mt-1 w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Phone</label>
              <input name="phoneNumber" value={f.phoneNumber} onChange={on} className="mt-1 w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700">Email</label>
              <input type="email" name="email" value={f.email} onChange={on} className="mt-1 w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Address</label>
              <input name="address" value={f.address} onChange={on} className="mt-1 w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
            </div>
          </div>
          <div>
